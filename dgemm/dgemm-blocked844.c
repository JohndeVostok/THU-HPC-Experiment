...
void mma844(const double *a, int lda, const double *b, int ldb, double *c, int ldc) {
    __m256d frag_c[2][4];
    __m256d frag_a[2];
    __m256d frag_b[2];
    ...
    for (int p = 0; p < 4; p++) {
        ...
        #pragma unroll
        for (int i = 0; i < 4; i++) {
            frag_b[i] = _mm256_broadcast_sd(&b[p*ldb+i]);
        }
        ...
        for (int j = 0; j < 4; j++) {
            #pragma unroll
            for (int i = 0; i < 2; i++) {
                frag_c[i][j] = _mm256_fmadd_pd(frag_a[i], frag_b[j], frag_c[i][j]);
            }
        }
    }
            ...
            for (int j = 0; j < lda; j += NR) {
                ...
                for (int i = 0; i < MIN(MC, lda-i1*MC); i += MR) {
                    ...
                    for (int k = 0; k < MIN(KC,lda-k1*KC); k += 4) {
                        mma844(buf_a + i * KC + k * MR, MR, buf_b + k * NR, NR, buf_c, MR);
                    }
                    for (int j0 = 0; j0 < MIN(NR,lda-j); j0++) {
                        for (int i0 = 0; i0 < MIN(MR,lda-i1*MC-i); i0++) {
